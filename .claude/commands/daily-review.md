...
- **`analysis.apology`** — hit_rate < 0.5 인 날에만 작성. 조건 불충족 시 필드 생략.
  ...
  - **톤 가이드**: 담담하지만 무겁게. "죄송합니다"로 끝나는 단순 사과가 아니라, 본인이 실제로 반성하고 있다는 게 느껴지는 문장. 자책이 지나치거나 감정 과잉도 금지.
  - **길이**: 3~5문장 (200~350자). 너무 짧으면 진정성 없어 보임, 너무 길면 변명처럼 읽힘.
  - **예시 (참고용, 그대로 쓰지 말 것)**:
    > "오늘 삼성전자와 현대차를 포함한 9개 종목에서 방향 예측이 빗나갔습니다. 실적 발표 당일 차익실현 압력을 구조적으로 과소평가한 것이 가장 큰 패착이었습니다. 고객 분들이 제 의견을 참고하셨다면, 오늘 하락장에서 불필요한 손실을 입으셨을 수 있습니다. 내일부터는 실적 확정 공시 당일의 예측에 보수적 기본값(priced_in=true 우선)을 적용하겠습니다. 신뢰를 회복하는 것은 말이 아니라 내일의 판단으로 증명하겠습니다."
