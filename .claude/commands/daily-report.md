...
## 어제 사용자 피드백 참고 (optional)

어제 발행된 페이지에서 사용자가 각 뉴스에 👍/👎 를 남겼을 수 있음. 수집된 피드백:

```bash
curl -s "https://k-ant-daily-quotes.yummyummyummy.workers.dev/feedback?date=$(date -v-1d +%Y-%m-%d 2>/dev/null || date -d 'yesterday' +%Y-%m-%d)" | .venv/bin/python -m json.tool
```

(Worker의 KV 바인딩이 없으면 503 — 그냥 skip)
...
```json
{
  ...
  "count": 4,
  "entries": [
    {"url": "...", "rating": "up",   "ts": 1729500000000, "stock": "000660"},
    {"url": "...", "rating": "down", "ts": 1729500100000, "stock": "005930"}
  ]
}
```

**활용 방법**:
- `👎 down` 마크된 기사 타입 = 사용자가 "노이즈"로 판단한 것 → **오늘 큐레이션에서 유사 패턴 회피**
- `👍 up` 마크된 기사 타입 = "유용" → **오늘 유사 특성 기사가 있으면 포함 우선**
- 예: 어제 "동반성장협의회" 관련 기사가 👎 다수였다면 → 오늘도 유사 운영 PR 성격 기사 제외
- 피드백이 없거나 적으면 그냥 skip. 무리하게 반영하지 말 것.
