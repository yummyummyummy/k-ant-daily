...
**펼친 body 순서**:
1. **📰 오늘의 뉴스** (material만 5건 + 더보기) — 메인 콘텐츠
2. 결정 근거 블록 (뉴스톤 · 간밤 신호 · 반영 여부 · 신뢰도 · rationale)
3. 🏁 리뷰 결과 (저녁 업데이트 후)
4. 🔬 심층 분석 (`deep_dive: true` 만)
5. 📋 공시
...
의사결정 보조도구 → 의사결정 도구 업그레이드를 위한 후속 기능:

1. **스파크라인** — 20일 종가 미니차트 (추세 맥락)
2. **실적 캘린더** — "이번 주 실적 발표하는 보유 종목" 하이라이트
3. **52주 고점·저점 거리** — "신고가 대비 -3%" 같은 context
4. **섹터 ETF 성과** — 섹터 카드에 대표 ETF 성과 연결
5. **누적 리뷰 대시보드** — 주간/월간 적중률, 신뢰도별 성과 트렌드
6. **컨센서스 vs 현재가** — 애널리스트 목표가 대비 괴리 (파이낸셜 데이터 소스 필요)
7. **매크로 이벤트 캘린더** — FOMC, 옵션 만기, 배당락 플래그
