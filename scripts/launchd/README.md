...
네 개의 LaunchAgent가 설치됨:
- `com.yummyummyummy.k-ant-daily.briefing`     — 평일 **07:30 KST** `/daily-report` (아침 예측)
- `com.yummyummyummy.k-ant-daily.nxt-snapshot` — 평일 **08:45 KST** `snapshot_nxt.py` (NXT pre-open 스냅샷 → summary.json baked)
...
**이전 `refresh` agent (10분 주기 뉴스/시세 재렌더 + git push) 는 제거됨.** 장중 뉴스 새로고침은 이제 브라우저가 직접 Cloudflare Worker `/stock-news` 엔드포인트를 5분 주기로 polling 해서 DOM 에 반영하는 방식 — git commit 발생 안 함. plist/wrapper 파일은 레포에 남겨두되 install.sh 가 설치하지 않음 (legacy 로 감지되면 자동 unload).
