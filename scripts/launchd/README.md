...
`check-results` 는 매 30분 fire 하지만 `scripts/pending_results.py` 로 **저렴하게 게이트**됨 — 결과 대기 이벤트(최근 4시간 내 종료 + result 비어있음)가 없으면 git/Claude 건드리지 않고 즉시 종료. 즉 이벤트 있는 날에만 실제 실행 → Claude 사용량 최소. dirty 작업 트리면 reset 안 하고 skip (작업 보호).
